package gf_eparking_registration_customer.error;
...
private boolean checkbirthdate()
{
	...
	try {
	    ...
	    exp.printStackTrace();
	    fehlermeldung.append("Bitte ein Datum eingeben ");
	    check = false;
	    error_text[4][1] = "";
	}
...
private boolean check_if_eighteen(Calendar cal_now, Calendar cal, StringBuilder fehlermeldung, java.util.Date date)
{
	...
	if((cal_now.get(Calendar.YEAR) - cal.get(Calendar.YEAR)) < 18)
	{
		check = false;
		fehlermeldung.append("Sie m¸ssen 18 sein");
		error_text[4][1] = "";
	}
	else if((cal_now.get(Calendar.YEAR) - cal.get(Calendar.YEAR)) == 18)
	{
		...
		else if(cal.get(Calendar.MONTH) == cal_now.get(Calendar.MONTH))
		{
			...
			{
			check = false;
			fehlermeldung.append("Das Mindestalter ist 17");
			error_text[4][1] = "";
			}
		}
	}
...
private boolean checkstreet()
{
	...
	{
		fehlermeldung = "Straﬂe darf nicht leer sein";
